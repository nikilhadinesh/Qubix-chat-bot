...
const typingEffect = (text, textElement, botMsgDiv) => {
    ...
    typingInterval = setInterval(() => {
        if (wordIndex < words.length) {
            ...
            scrollToBottom(); // call scroll after every word
        } else {
...
console.log("Scroll Height:", chatsContainer.scrollHeight);
...
// Function to show/hide wallpaper based on chatbot state
const toggleWallpaperVisibility = (isVisible) => {
    homeWallpaper.style.opacity = isVisible ? "1" : "0";
};

// Call this function when the chatbot is responding
document.body.addEventListener("classChange", () => {
    const isBotResponding = document.body.classList.contains("bot-responding");
    toggleWallpaperVisibility(!isBotResponding);
});
...
const handleFormSubmit = (e) => {
   ...
   const userMessage = promptInput.value.trim();
    ...
    toggleWallpaperVisibility(false);
    fileUploadWrapper.classList.remove("active", "img-attached", "file-attached");

    //Generate user message HTML with optional attachment
    const userMsgHTML = `
    ...
    setTimeout(() => {
        //Generate bot message HTML and add in the chats containers after 600ms
    const botMsgHTML = `<img src="/img/moon-removebg-preview.png" class="avatar"><p class="message-text">Just a sec...</p>`;
    const botMsgDiv = createMsgElement(botMsgHTML, "bot-message", "loading");
    chatsContainer.appendChild(botMsgDiv);
    scrollToBottom();
    generateResponse(botMsgDiv ,userMessage);
    toggleWallpaperVisibility(true);
    } ,600);
}
